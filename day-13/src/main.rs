...
fn main() -> Result<(), String> {
    ...
    let (earliest_departure, bus_ids) = parse_without_x(&content)?;
...
fn find_earliest_bus_after_timestamp(timestamp: u64, bus_ids: &[u64]) -> Option<(u64, u64)> {
    bus_ids
        .iter()
...
fn parse_without_x(input: &str) -> Result<(u64, Vec<u64>), String> {
    ...
    let bus_ids = parse_bus_ids_without_x(bus_ids_string)?;

    Ok((earliest_departure, bus_ids))
}

fn parse_bus_ids_without_x(line: &str) -> Result<Vec<u64>, String> {
    line.split(',')
        .filter(|s| s != &"x")
        .map(|s| {
            s.parse()
